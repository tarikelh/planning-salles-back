...
class LocationServiceTest {
	...
	void shouldReturnCount() {
		when(locationRepository.countByCityContaining(any(String.class))).thenReturn((long) lList.size());
	...
	@Test
	void shouldGetLocationsFromDG2() throws Exception {
		String body = "[{\"id\":1,\"name\":\"Paris\",\"boolean\":true,\"country\":\"FR\"}]";
		...
		when(restTemplate.exchange(any(URI.class), any(HttpMethod.class), any(HttpEntity.class), any(Class.class)))
				.thenReturn(res);
		when(locationMapper.locationDG2DtoToLocation(any(LocationDG2Dto.class))).thenReturn(lList.get(0));
		when(locationRepository.findById(lList.get(0).getId())).thenReturn(opLocation);
		when(locationRepository.saveAndFlush(lList.get(0))).thenReturn(lList.get(0));

		assertDoesNotThrow(() -> locationService.fetchAllDG2Locations("emailDG2", "passwordDG2"));
	}
}
