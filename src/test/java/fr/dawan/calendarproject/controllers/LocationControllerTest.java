...
public class LocationControllerTest {
	...
	@Test
	public void shouldFetchLocationById() throws Exception {
		final long locId = 2;
	...
	public void ShouldDeleteLocationById() throws Exception {
		...
		String res = mockMvc.perform(delete("/api/locations/{id}", locId))
	...
	public void ShouldCreateNewLocation() throws Exception {
		...
		LocationDto newLoc = new LocationDto(0, "Lyon", "#C33E42", 0);
		LocationDto resMock = new LocationDto(4, "Lyon", "#C33E42", 0);
		...
		mockMvc.perform(post("/api/locations")
				...
				.accept(MediaType.APPLICATION_JSON).content(newLocJson))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.id", is(4)))
				.andExpect(jsonPath("$.city", is(newLoc.getCity())))
	...
	public void shouldUpdateLocation() throws Exception {
		...
		mockMvc.perform(put("/api/locations")
				...
				.andExpect(jsonPath("$.city", is(locs.get(0).getCity())))
	...
	public void shouldReturn404WhenUpdateWithWrongId() throws Exception {
		...
		LocationDto wrongIdLoc = new LocationDto(12, "Nantes", "#9859C6", 0);
		...
		String res = mockMvc.perform(put("/api/locations").contentType(MediaType.APPLICATION_JSON).characterEncoding("utf-8")
